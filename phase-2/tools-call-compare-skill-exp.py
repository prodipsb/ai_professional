...
# ======================================================
# Add a second function: skill matching
# ======================================================


def check_skill_match(candidate_skills, required_skills):
    """
    Compares two lists of skills (case-insensitive) and returns 
    what matches and what's missing.
    
    """

    # Normalize both lists to lowercase for fair comparison
    candidate_set = set(s.lower().strip() for s in candidate_skills)
    required_set = set(s.lower().strip() for s in required_skills)

    matched = candidate_set & required_set              # intersection = common skills
    missing = required_set - candidate_set              # required skills NOT in candidate's list

    match_percentage = round(len(matched) / len(required_set) * 100, 1) if required_set else 0
...
tools = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Compares a candidate's skill against a job's required  skills and returns matched/missing skills with a match percentage",
            "parameters": {
                "type":"object",
                "properties": {
                    "candidate_skills": {
                    ...
                    "required_skills": {
                        "type": "array",
                        "items": {"type": "string"},
                        "description": "List of skills the job requires"
                    }
                },
...
# user_question = "I know Python, Docker, and SQL. The job needs Python, Docker, Kubernetes, and AWS. What's my match?"

user_question = "The job requires 5 years experience, I have 7 years. Do I qualify?"
...
if response_message.tool_calls:
    ...
    elif function_name == "check_skill_match":
        result = check_skill_match(**fucntion_args)
